...
export function calculateOCAM(cat1_1, cat1_2, cat2_1, cat2_2) {
  ...
  // Step 1: Dynamic weighted calculation (60% to higher mark, 40% to lower mark)
  const maxMark = Math.max(cat1, cat2);
  const minMark = Math.min(cat1, cat2);
  const maxWeighted = maxMark * 0.60;
  const minWeighted = minMark * 0.40;
  
  // Step 2 & 3: Final OCAM Mark & Rounded
  const rawOcam = maxWeighted + minWeighted;
