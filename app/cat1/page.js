...
export default async function CAT1Page({ searchParams }) {
  ...
  // Filter to only include records that have at least one CAT 1 entry mark
  markEntries = markEntries.filter(entry => entry.cat1Entry1 !== null || entry.cat1Entry2 !== null);

  const mismatchesCount = markEntries.filter(entry => 
    entry.cat1Entry1 !== null && entry.cat1Entry2 !== null && entry.cat1Entry1 !== entry.cat1Entry2
  ).length;

  const regMismatchesCount = markEntries.filter(entry => 
    (entry.cat1Entry1 !== null && entry.cat1Entry2 === null) || (entry.cat1Entry1 === null && entry.cat1Entry2 !== null)
  ).length;

  const crossRegMismatchesCount = markEntries.filter(entry => 
    entry.cat2Entry1 === null && entry.cat2Entry2 === null
  ).length;

  let tableData = markEntries.map(entry => {
    const diff = (entry.cat1Entry2 || 0) - (entry.cat1Entry1 || 0);
    const hasMismatch = entry.cat1Entry1 !== null && entry.cat1Entry2 !== null && entry.cat1Entry1 !== entry.cat1Entry2;
    const hasRegMismatch = (entry.cat1Entry1 !== null && entry.cat1Entry2 === null) || (entry.cat1Entry1 === null && entry.cat1Entry2 !== null);
    const hasCrossRegMismatch = entry.cat2Entry1 === null && entry.cat2Entry2 === null;
    return {
      id: entry.id,
      studentId: entry.studentId,
      courseCode: entry.module.courseCode,
      cat1Entry1: entry.cat1Entry1,
      cat1Entry2: entry.cat1Entry2,
      diff,
      hasMismatch,
      hasRegMismatch,
      hasCrossRegMismatch,
  ...
  const exportData = tableData.map(entry => ({
    ...
    'Entry 1': entry.cat1Entry1 ?? '-',
    'Entry 2': entry.cat1Entry2 ?? '-',
    'Difference': entry.diff,
    'Status': entry.hasMismatch ? 'Mismatch' : entry.hasRegMismatch ? 'Unmatched Registration' : entry.hasCrossRegMismatch ? 'Missing in CAT 2' : 'No Change'
  }));
  ...
  return (
    <div className={styles.dashboardContainer}>
      <header className={styles.header}>
        <div>
          <h1 className={styles.title}>CAT 1 Comparison</h1>
          <p className={styles.subtitle}>Detailed comparison between CAT 1 Entry 1 and Entry 2.</p>
        </div>
        <div className={styles.actions} style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          <SearchInput placeholder="Search Reg. No..." />
          <ExportFilteredData data={exportData} filename="CAT 1 Comparison Report" />
        </div>
      </header>
      ...
      {(mismatchesCount > 0 || regMismatchesCount > 0 || crossRegMismatchesCount > 0) && (
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <strong>CAT 1 Verification Alert:</strong> We detected
              ...
              {crossRegMismatchesCount > 0 && <span> <strong>{crossRegMismatchesCount}</strong> registration numbers missing in CAT 2 dataset</span>}
                ...
                {tableData.length > 0 ? tableData.map(entry => {
                  return (
                    ...
                    >
                      ...
                      <td>{entry.cat1Entry1 ?? '-'}</td>
                      <td>{entry.cat1Entry2 ?? '-'}</td>
                      ...
                      <td>
                        {entry.hasMismatch ? (
                          ...
                          <span className={styles.statusPending} style={{background: 'rgba(239, 68, 68, 0.15)', color: '#f87171', border: '1px solid rgba(239, 68, 68, 0.3)'}}>Missing in CAT 2</span>
                        ) : (
                  ...
                  <tr>
                    <td colSpan={isCenterEnabled ? "7" : "6"} style={{textAlign: "center", color: "var(--text-muted)"}}>No CAT 1 data available. Please upload files.</td>
                  </tr>
                )}
              </tbody>
            </table>
